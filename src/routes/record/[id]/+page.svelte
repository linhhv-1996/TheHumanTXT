<script>
        ...
        try {
            ...
            unsubscribeDb = subscribeToRecord(thoughtId, (data) => {
                ...
                if (engine.phase === "drift" && !engine.myParticle) {
                    engine.init(
                        thoughtId,
                        thoughtData.code || thoughtData.text,
                    );
                    engine.start();
                }
            });
    ...
    function handleShareVoice() {
        ...
        if (navigator.share) {
            navigator
                .share({ title: "A commit in The Human TXT", url })
                .catch(() => {});
        } else {
...
{#if errorMsg}
    <div id="loading" class="show" style="flex-direction: column;">
        <p class="loading-text" style="color: #aa5555; animation: none;">
            {errorMsg}
        </p>
        <button
            class="btn-ghost"
            style="margin-top: 24px; pointer-events: all;"
            onclick={() => goto("/")}>← Return to Canvas</button
        >
    </div>
{/if}
...
{#if showLoading && !errorMsg}
    <div
        id="loading"
        class:hidden={engine?.phase !== "drift"}
...
<div id="you-label" style={youLabelStyle} class:show={showYouLabel}>
    — the thought
</div>
...
{#if showDetailView && thoughtData}
    <div id="voice-card" class="show" transition:fade={{ duration: 400 }}>
        ...
        <div class="identity-row">
            ...
            {/if}
            {#if thoughtData.link}
                {#if thoughtData.name || thoughtData.location}<span
                        class="id-sep">·</span
                    >{/if}
                <a
                    class="id-social"
                    href={thoughtData.link.startsWith("http")
                        ? thoughtData.link
                        : `https://${thoughtData.link}`}
                    target="_blank"
                    rel="noopener">{thoughtData.link}</a
                >
            {/if}
        </div>
        ...
        <div class="onchain-row">
            ...
            {#if thoughtData.status === "pending" || !thoughtData.txHash}
                <span
                    class="onchain-item"
                    style="color: #d1b866; animation: blink 1.5s infinite;"
                >
            ...
            {:else}
                <span class="onchain-item">
                    <a
                        href="https://basescan.org/tx/{thoughtData.txHash}"
                        target="_blank"
                        rel="noopener"
                    >
                        {thoughtData.txHash.slice(
                            0,
                            10,
                        )}...{thoughtData.txHash.slice(-6)}
                    </a>
                </span>
                <span class="onchain-sep">·</span>
                <span class="onchain-item">
        ...
        <div class="cta-row">
            <button class="btn-ghost" onclick={handleShareVoice}
                >Share this commit</button
            >
